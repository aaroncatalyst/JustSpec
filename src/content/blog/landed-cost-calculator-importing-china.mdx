...
We'll use a product we know well: a 500ml HDPE water bottle, $2.00 FOB Shenzhen, 10,000 units. This product's HTS code lands it in a category with a moderate combined tariff rate — we'll use ~32% (MFN 6.5% + Section 301 25%, plus a small emergency/reciprocal surcharge) as an illustrative baseline. Note that the emergency surcharge is in legal flux and must be checked at entry, so [verify your specific current rate at lgistics.ai](https://lgistics.ai) before using any number you find in an article.
...
Combined tariff rate for this product: ~32% (MFN 6.5% + Section 301 25%, plus a small emergency/reciprocal surcharge — verify current)
...
Before you finalize your import calculations, [look up your exact HTS code and current combined rate](https://lgistics.ai). Section 301 tariff rates vary by product subheading, and the emergency-tariff layer on top has been changing with the ongoing litigation — neither is captured in any single percentage. Using the wrong rate leads to seriously wrong cost models.
...
**Forgetting the emergency-tariff layer.** In 2025 an IEEPA surcharge was stacked on top of Section 301; the Supreme Court struck it down in February 2026 and a Section 122 replacement is now being litigated. Whatever surcharge is in effect the day your goods clear is a real, moving line item on top of Section 301 — don't leave it out, and don't assume last quarter's number. See [what happened to the IEEPA tariffs](/blog/ieepa-tariffs-china-2025-2026).
